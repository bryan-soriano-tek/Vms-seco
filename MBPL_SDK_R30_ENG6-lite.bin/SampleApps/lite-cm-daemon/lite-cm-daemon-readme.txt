...
lite-cm-daemon (the application) is a sample application/utility that acts as a daemon to monitor connections over Sierra Wireless Modems based on Qualcomm chipsets. 
The app initializes the WDS QMI service, sets the IP address, routing and the MTU for the network interface based on data connections status changes.  
Must be used when establising data calls using AT!SCACT (supported by some of the Sierra Wireless modems). 
lite-cm-daemon is needed becasue the qmi_wwan driver does NOT intialize the WDS QMI service, which used to be done in the GobiNet driver.
...
App usage: 
  ...
  <appName> [[[-m|-q] [-d <QMI/MBIM device path>]]|[-r]] [-n] [-V] [-h]
...
Automatic mode (default)
  sudo ./lite-cm-daemonhostx86_64

Router mode (qmux-router is required to be running)
  ./lite-cm-daemonhostx86_64 -r

MBIM mode
  sudo ./lite-cm-daemonhostx86_64 -m

QMI mode
  sudo ./lite-cm-daemonhostx86_64 -q

Non-daemon mode
  sudo ./lite-cm-daemonhostx86_64 -n
